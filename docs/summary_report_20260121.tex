...
14日間 & 16 & 1,430.4 & \textbf{1,398.8} & \textbf{25.1} & \textbf{10.8\%} \\
\midrule
...
今週の実験結果を卒業論文（research\_report.tex）に追加済み：
\begin{itemize}
    \item 結果セクション：「年間一括最適化との比較」「制御ホライズンと長期予測期間の影響」を新規追加
    \item 考察セクション：「長期予測期間による改善効果」を追加
    \item 結論セクション：7項目目として長期予測期間と制御ホライズンの知見を追加
\end{itemize}

\section{今後の課題・次週の予定}

\begin{itemize}
    \item 論文の全体校正と図表の最終調整
    \item 予測誤差がコストに与える影響の検討（現在は完全予見を仮定）
    \item 論文を35ページに拡充するための追加コンテンツ検討
\end{itemize}
