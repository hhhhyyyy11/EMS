...
本研究では，不確実性を伴う環境下での運用計画立案にローリング計画法（Rolling Horizon Approach）と混合整数線形計画法（MILP: Mixed Integer Linear Programming）を組み合わせて適用する．
...
本章ではシミュレーション結果を述べる．まず代表的な日のデータでシステムの基本的な運用パターンを示し，次に蓄電池容量と料金プランの関係性，予測期間の影響，年間一括最適化との比較の順に検証結果を示す．
...
\subsection{制御ホライズンと長期予測期間の影響}

前節で確認されたローリング計画法と年間一括最適化のコスト差を縮小するため，予測期間の延長と制御ホライズンの調整による効果を検証した．制御ホライズンとは，1回の最適化計算で採用する決定ステップ数であり，デフォルトの1（30分ごとに再計画）から8（4時間ごとに再計画）まで変化させた．

\subsubsection{制御ホライズンの影響}

予測期間96ステップ（48時間）において，制御ホライズンを変化させた結果を表\ref{tab:control_horizon}に示す．
...
\noindent
北海道電力基本プランではコストへの影響がなく，市場価格連動プランでも最大0.1\%程度の差に留まった．一方，計算時間は制御ホライズン8で基準の約\textbf{12\%}まで短縮された．この結果は，制御ホライズンの延長が計算効率の大幅な改善をもたらす一方，解の品質への影響は限定的であることを示す．

\subsubsection{長期予測期間と制御ホライズンの組み合わせ}

制御ホライズンの延長により計算時間を抑制しつつ，予測期間を192ステップ（4日間），384ステップ（8日間），672ステップ（14日間）まで延長した結果を表\ref{tab:long_horizon}に示す．

\begin{table}[H]
\centering
\caption{長期予測期間と制御ホライズンの影響（蓄電池860kWh）}
...
\begin{enumerate}
    ...
    \item \textbf{計算時間の抑制}：制御ホライズン8を用いることで，14日間予測でも計算時間を48.5分に抑制できた．これは48時間予測・制御ホライズン1（33.2分）と同程度である．

    \item \textbf{制御ホライズンの影響}：同一予測期間では，制御ホライズンによるコスト差は0.5万円未満であり，計算効率の改善効果が支配的であった．
\end{enumerate}
...
21日間予測（制御ホライズン24）において蓄電池容量1290kWhで実験したところ，北海道電力基本プラン1,400.8万円に対し市場価格連動プラン1,399.1万円と，わずか1.7万円差でほぼ同等となった．48時間予測では540kWh以上で北海道電力基本プランが有利であったが，21日間予測により，大容量蓄電池でも市場連動プランの競争力が回復することが確認された．
...
\begin{enumerate}
    ...
    \item \textbf{実行}：最初の$m$ステップ（制御ホライズン）のみを実行
...
前節では按分手法の数理的問題を述べたが，本節ではより本質的な問題として「リスクの偏在性」と「認識の非対称性」について論じる．これらの概念は，第\ref{sec:bounded_rationality}節で述べた限定合理性の議論を補完し，市場価格連動プランにおける契約電力増大のメカニズムをより深く理解するための鍵となる．
...
図\ref{fig:monthly_demand}に示した月別需要パターンから明らかなように，本研究の対象施設では12月・1月の冬季に需要が集中し，特に1月6日8:30に年間最大需要264.0kWを記録した．すなわち，「年間最大買電電力の発生リスク」は冬季の特定期間に偏在している．

この偏在性を定量化すると：

\begin{itemize}
    \item 冬季（12月〜2月）：年間需要の約35\%，最大需要発生確率は約70\%以上
    \item 夏季（6月〜8月）：年間需要の約18\%，最大需要発生確率は極めて低い
    \item 中間期（3月〜5月，9月〜11月）：最大需要発生確率は低〜中程度
\end{itemize}

均等配分の$w_{\mathrm{basic}}$は，この偏在性を反映していない．結果として，リスクの低い時期（夏季）には過剰なペナルティを，リスクの高い時期（冬季）には不十分なペナルティを課すことになる．
...
\begin{enumerate}
    ...
    \item \textbf{長期予測期間と制御ホライズン}：制御ホライズンを4時間（8ステップ）に延長することで計算時間を約88\%削減しつつ，解の品質への影響は0.1\%未満に抑制できた．また，予測期間を14日間（672ステップ）まで延長することで，市場価格連動プランの年間一括最適化との差が22\%から11\%に縮小し，8日間以上の予測では市場連動プランの優位性が回復した．
\end{enumerate}
