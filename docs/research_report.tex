...
本論文の構成は以下の通りである．第2章ではシステム構成と制約条件を述べ，第3章で最適化手法（ローリング計画法およびMILP定式化）を説明する．第4章で実験設定を示し，第5章でシミュレーション結果を報告する．第6章では結果に対する考察を行い，料金プラン選択と蓄電池容量の関係，ローリング計画法の限界，予測の不確実性に対するロバスト性などを議論する．第7章で結論を述べる．
...
なお，本研究では\textbf{制御ホライズン}$m$を導入する．制御ホライズンとは，1回の最適化計算で実際に採用・実行する決定ステップ数を指す．基本設定では$m=1$（30分ごとに再計画）であるが，$m$を増加させることで計画頻度を下げ，計算負荷を削減できる．例えば$m=8$とすると，4時間ごとに再計画を行う．予測期間$H$と制御ホライズン$m$の関係を以下に示す：

\begin{itemize}
    \item \textbf{予測期間$H$}：最適化問題の時間的視野（何ステップ先まで考慮するか）
    \item \textbf{制御ホライズン$m$}：最適化結果のうち実行するステップ数（再計画の頻度）
\end{itemize}

\noindent
一般に$m \leq H$であり，$m=1$が最も頻繁な再計画（高精度・高計算負荷），$m=H$が最も少ない再計画（低精度・低計算負荷）に対応する．
