...
\begin{table}[H]
\centering
\caption{予測期間24時間と48時間の比較（年間統計，市場価格連動プラン，蓄電池860kWh）}
...
PV利用率 [\%] & 100.00 & 100.00 \\
PV余剰量 [kWh] & 0 & 0 \\
契約電力 [kW] & 237.00 & 221.56 \\
年間買電量 [kWh] & 547,810 & 547,990 \\
年間コスト [円] & 15,872,724 & 15,396,972 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{PV利用率}：24時間予測・48時間予測ともに100.00\%を達成し，PV余剰は発生しなかった．

    \item \textbf{契約電力削減効果}：48時間予測（221.56kW）は24時間予測（237.00kW）より15.44kW（6.5\%）低く，基本料金の削減に寄与する．

    \item \textbf{年間コスト削減効果}：48時間予測により年間約47.6万円（3.0\%）のコスト削減を達成した．これは主に契約電力の低下による基本料金削減によるものである．

    \item \textbf{予測期間の影響}：24時間予測では近視眼的な運用となり，価格が安い時間帯への買電集中が過度に進む．48時間予測により2日間の需給・価格パターンを考慮した戦略的運用が可能となり，契約電力の抑制と経済性向上を両立できる．
\end{enumerate}
...
表\ref{tab:horizon_comparison_detail}に，蓄電池容量860kWhにおける予測期間$H=48$（24時間）と$H=96$（48時間）の両プランでの比較結果を示す．
...
\noindent
予測期間を48時間に延長することで，両プランとも年間コストが削減された．特に市場価格連動プランでは約47.6万円（3.0\%）の削減効果があり，契約電力も237.00kWから221.56kWへと15.44kW低下した．これは，より長い予測期間により，価格変動を考慮した戦略的な買電タイミングの最適化が可能になったためである．北海道電力基本プランでも約8.4万円（0.6\%）の削減と，契約電力2.77kWの低下が確認された．
