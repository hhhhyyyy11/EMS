...
2024年の年間システム運用統計をTable~\ref{tab:system_stats}に示す．なお，PV自給率は「PV発電による消費量 $\div$ 総需要 $\times$ 100 [\%]」，PV利用率は「実際に使用したPV発電量 $\div$ 発電可能量 $\times$ 100 [\%]」で定義される．
...
予測期間96ステップ（48時間）において，再計画間隔を変化させた結果をTable~\ref{tab:control_horizon}に示す．
...
\midrule
21 days (1008) & 24 & 1,429.9 & \textbf{1,380.2} & 27.6 & \textbf{9.3\%} \\
\midrule
...
蓄電池860kWhの運用データを月別に分析した結果をTable~\ref{tab:monthly_analysis}に示す．ここで，買電抑制率は$(\text{需要の最大電力} - \text{買電の最大電力}) / \text{需要の最大電力} \times 100$で算出した値であり，蓄電池によって買電の最大電力がどの程度抑制されたかを示す．
...
Fig.\ref{fig:price_charge_scatter}に，JEPX価格と充放電量の関係を散布図として示す．この図は市場価格連動プランにおいて，最適化アルゴリズムがどのような価格帯で充放電を行っているか，その「ポリシー関数」を可視化したものである．
...
JEPXスポット価格の月別統計をTable~\ref{tab:price_monthly}に示す．ここで，「$>$北電率」は市場価格が北海道電力基本プランの電力量料金（21.51円/kWh）を上回る時間帯の割合を示す．
...
特に注目すべきは，本研究で観察された「大容量蓄電池での契約電力増大」現象（Table~\ref{tab:capacity_comparison_market_detail}）が，完全予見下でさえ発生しているという点である．予測誤差が加わる実運用環境では，この現象がさらに悪化する可能性が高い．
