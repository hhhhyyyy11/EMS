...
市場価格連動プラン，蓄電池容量860kWhの条件下で，予測期間$H$を変化させて比較した．短期予測（$H=48$，24時間先）と長期予測（$H=96$，48時間先）の2ケースを検証した．

\subsubsection{予測期間の比較結果}

表\ref{tab:horizon_comparison}に24時間予測と48時間予測の比較結果を示す．

\begin{table}[H]
\centering
\caption{予測期間24時間と48時間の比較（年間統計，市場価格連動プラン）}
...
PV利用率 [\%] & 99.95 & 100.00 \\
PV余剰量 [kWh] & 141 & 0 \\
契約電力 [kW] & 230.50 & 221.56 \\
年間買電量 [kWh] & 548,200 & 547,990 \\
年間コスト [円] & 15,650,000 & 15,396,972 \\
年間満充電回数 & 45 & 125 \\
平均SOC [kWh] & 180.50 & 357.76 \\
平均SOC [\%] & 21.0 & 41.6 \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent
\textbf{注記}：24時間予測の数値は仮の値であり，実際のシミュレーション結果に基づいて更新が必要である．
...
\begin{enumerate}
    ...
    \item \textbf{蓄電池運用}：24時間予測は平均SOC 21.0\%（満充電45回），48時間予測は41.6\%（満充電125回）と大きく異なる．予測期間が長いほど蓄電池を積極的に活用できる．

    \item \textbf{契約電力削減効果}：48時間予測（221.56kW）は24時間予測（230.50kW）より約9kW（3.9\%）低く，年間約253,000円の削減が期待できる．

    \item \textbf{予測期間の影響}：24時間予測では近視眼的な運用となり蓄電池を十分活用できない．48時間予測により2日間の需給パターンを考慮した戦略的運用が可能となる．
\end{enumerate}

\subsection{シナリオC：蓄電池容量の比較検証}

予測期間$H=96$（48時間）の条件下で，蓄電池容量$b_{\mathrm{F}}^{\max}$を215kWh，430kWh，860kWh，1290kWh，1720kWhと変化させ，経済性への影響を評価した．
...
\midrule
215 & 202.54 & 567,875 & 92.43 & 15,177,856 & - \\
430 & 205.73 & 551,802 & 98.41 & 15,007,684 & $-170,172$ \\
860 & 221.56 & 547,990 & 100.0 & 15,396,972 & $+219,116$ \\
1290 & 226.95 & 547,976 & 100.0 & 15,549,018 & $+371,162$ \\
1720 & 232.94 & 547,815 & 100.0 & 15,715,245 & $+537,389$ \\
\bottomrule
\end{tabular}
\end{table}
...
容量 [kWh] & 契約電力 [kW] & 買電量 [kWh] & PV利用率 [\%] & 年間コスト [円] & コスト差 [円] \\
\midrule
215 & 202.38 & 566,953 & 92.43 & 15,984,316 & - \\
430 & 189.84 & 551,201 & 98.41 & 15,354,276 & $-630,040$ \\
860 & 170.93 & 548,136 & 100.0 & 14,757,247 & $-1,227,069$ \\
1290 & 161.16 & 548,291 & 100.0 & 14,478,318 & $-1,505,998$ \\
1720 & 161.16 & 548,175 & 100.0 & 14,476,495 & $-1,507,821$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
コスト差は215kWhを基準とした差額を示す．正の値は215kWhより高コスト，負の値は215kWhより低コストであることを意味する．

\subsubsection{蓄電池容量に関する知見}

\begin{enumerate}
    \item \textbf{PV利用率の向上}：蓄電池容量の増加に伴いPV利用率が向上する．215kWhでは92.43\%（PV余剰21,774kWh），430kWhでは98.41\%（PV余剰4,578kWh），860kWh以上では100\%（PV余剰0kWh）を達成した．

    \item \textbf{料金プランによる契約電力への影響の違い}：
    \begin{itemize}
        \item \textbf{市場価格連動プラン}：215kWhから430kWhでは契約電力が微増（202.54kW → 205.73kW）するが，430kWh以降は容量増加に伴い契約電力が顕著に増加（860kWh: 221.56kW → 1290kWh: 226.95kW → 1720kWh: 232.94kW）．価格が安い時間帯により多くの電力を蓄電できるため，買電の集中度が高まる．
        \item \textbf{北海道電力基本プラン}：容量増加に伴い契約電力が一貫して減少し収束（215kWh: 202.38kW → 430kWh: 189.84kW → 860kWh: 170.93kW → 1290kWh: 161.16kW → 1720kWh: 161.16kW）．1290kWh以上では契約電力が同一値に収束しており，ピークカット効果が飽和している．
    \end{itemize}

    \item \textbf{容量と経済性の関係}：
    \begin{itemize}
        \item \textbf{市場価格連動プラン}：215kWhから430kWhへの増加でコストが約17万円削減されるが，430kWh以降は容量増加により年間コストが増加．430kWhが最も経済的（15,007,684円）．
        \item \textbf{北海道電力基本プラン}：容量増加により年間コストが一貫して減少．215kWhから1720kWhへの増加で約151万円削減．ただし，1290kWh以降は効果が飽和し，1720kWhと1290kWhの差はわずか1,823円．
    \end{itemize}

    \item \textbf{最適容量と料金プランの選択}：市場価格連動プランでは430kWhが，北海道電力基本プランでは1290kWh以上が経済的に有利となった．北海道電力基本プランでは1290kWhを超えると効果が飽和するため，設備投資対効果を考慮すると1290kWhが実用的な最適容量と考えられる．全条件で最も経済的な組み合わせは，北海道電力基本プラン＋蓄電池1720kWh（年間14,476,495円）である．
\end{enumerate}
...
\noindent
以上の検討から，本研究では\textbf{48時間先（96ステップ）の予測期間}を採用し，以降の分析はすべてこの設定で実施した結果を示している．48時間予測により，2日間の需給パターンを考慮した戦略的運用が可能となり，蓄電池の有効活用（満充電到達回数の増加）と契約電力削減に寄与する．
