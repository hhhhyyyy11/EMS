...
\begin{titlepage}
    ...
    {\fontsize{20pt}{30pt}\selectfont 電力グリッドのエネルギー需給マネジメントに\\[0.5em]おけるローリング計画モデル}
...
\begin{tikzpicture}[scale=0.8]
    % 蓄電池容量（変化）
    \node[anchor=east] at (-0.5,0) {\textbf{蓄電池容量}};
    \draw[thick] (0,0) -- (5,0);
    ...
    \node[red, above] at (2.5,0.3) {\small \textbf{変化させる}};

    % 予測期間（固定）
    \node[anchor=east] at (-0.5,-1.5) {予測期間$H$};
    \draw[gray, thick] (0,-1.5) -- (5,-1.5);
    \fill[blue] (2.5,-1.5) circle (0.15);
    \node[below, gray] at (2.5,-1.7) {\small 固定: 48時間};

    % 再計画間隔（固定）
    \node[anchor=east] at (-0.5,-2.5) {再計画間隔$m$};
    \draw[gray, thick] (0,-2.5) -- (5,-2.5);
    \fill[blue] (0.5,-2.5) circle (0.15);
    \node[below, gray] at (0.5,-2.7) {\small 固定: 30分};
\end{tikzpicture}
\caption{本節の比較軸：蓄電池容量を0〜1720kWhで変化させ，予測期間と再計画間隔は固定．}
...
\begin{table}[H]
\centering
\caption{蓄電池容量別の年間コスト比較（両プラン，$H=96$，$m=1$）}
...
\begin{table}[H]
\centering
\caption{蓄電池導入効果の比較（蓄電池0kWh vs 860kWh，$H=96$，$m=1$）}
...
\begin{tikzpicture}[scale=0.8]
    % 蓄電池容量（固定）
    \node[anchor=east] at (-0.5,0) {蓄電池容量};
    \draw[gray, thick] (0,0) -- (5,0);
    \fill[blue] (2.5,0) circle (0.15);
    \node[below, gray] at (2.5,-0.2) {\small 固定: 860kWh};

    % 予測期間（変化）
    \node[anchor=east] at (-0.5,-1.5) {\textbf{予測期間$H$}};
    ...
    \node[red, above] at (2.5,-1.2) {\small \textbf{変化させる}};

    % 再計画間隔（固定）
    \node[anchor=east] at (-0.5,-3) {再計画間隔$m$};
    \draw[gray, thick] (0,-3) -- (5,-3);
    \fill[blue] (0.5,-3) circle (0.15);
    \node[below, gray] at (0.5,-3.2) {\small 固定: 30分};
\end{tikzpicture}
\caption{本節の比較軸：予測期間$H$を24〜72時間で変化させ，蓄電池容量と再計画間隔は固定．}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[below] at (0,-1.7) {\scriptsize 4日};
    \node[below] at (5,-1.7) {\scriptsize 21日};
    \draw[red, very thick, <->] (0.1,-1.2) -- (4.9,-1.2);
    \node[red, above] at (2.5,-1.2) {\small \textbf{変化させる}};

    % 再計画間隔（変化）
    \node[anchor=east] at (-0.5,-3) {\textbf{再計画間隔$m$}};
    \draw[thick] (0,-3) -- (5,-3);
    \foreach \x in {0, 2.5, 5} {
        \draw[thick] (\x,-3.1) -- (\x,-2.9);
    }
    \node[below] at (0,-3.2) {\scriptsize 30分};
    \node[below] at (5,-3.2) {\scriptsize 12時間};
    \draw[red, very thick, <->] (0.1,-2.7) -- (4.9,-2.7);
    \node[red, above] at (2.5,-2.7) {\small \textbf{変化させる}};
\end{tikzpicture}
\caption{本節の比較軸：予測期間$H$（4〜21日間）と再計画間隔$m$（30分〜12時間）の両方を変化させる．}
